...
public class GUIMain{
    ...
    //connection to the socket server, used to send sensor/button/EMS events out
...
//        trafficLights.add(new TrafficLightVisual(redLight, yellowLight, greenLight));
    ...
    private void createCar(int id, GUILane lane, Bearing bearing, int laneNumber, boolean EMS) {
        ...
        // set up turning: lane 0/2 will pivot into the new road once they
        // reach the merge point, landing in a RANDOM lane on that road
        // (not always the middle/straight lane); lane 1 always goes straight
        if (laneNumber != 1) {
            Bearing pendingBearing = computeTurnBearing(bearing, laneNumber);
            int destinationLane = random.nextInt(LANES_PER_DIRECTION);
            double mergeThreshold = getMergeCoordinate(pendingBearing, destinationLane);
            carVisual.setTurnInfo(pendingBearing, mergeThreshold, destinationLane);
        }
    ...
    // lane 0 = left turn, lane 2 = right turn (lane 1 = straight, never reaches turn logic)
    private Bearing computeTurnBearing(Bearing entry, int laneNumber) {
        boolean isLeft = laneNumber == 0;
        return switch (entry) {
    ...
    // returns the coordinate for a given lane on the destination road, using
    // the same offset pattern as positionCar() so a turning car lands exactly
    // where a car that spawned in that lane naturally would
    private double getMergeCoordinate(Bearing bearing, int laneNumber) {
        double roadLeft = (WINDOW_WIDTH - ROAD_WIDTH) / 2;
        double roadTop = (WINDOW_HEIGHT - ROAD_WIDTH) / 2;
        return switch (bearing) {
    ...
    private void moveCar(CarVisual carVisual) {
        ...
        Timeline timeline = new Timeline(new KeyFrame(Duration.millis(16), event -> {
            ...
            if (!carVisual.hasTurned() && carVisual.getLaneNumber() != 1) {
                ...
                if (crossedMerge) {
                    ...
                    carVisual.setLaneNumber(carVisual.getDestinationLane());
            ...
            // fire sensor events over the socket the first time this car's
            // sensor activates, then again once it clears the stop-line zone
            GUICar logicCar = carVisual.getCar();

            if (logicCar.isSensorActive() && !carVisual.isSensorEventSent()) {
                carVisual.setSensorEventSent(true);
                if (server != null) {
                    GUILane sensorLane = getLane(carVisual.getEntryBearing(), carVisual.getEntryLaneNumber());
                    if (sensorLane != null) {
                        server.sendEvent(new SimulatorEvent(TLCCommand.VEHICLE_DETECTED, String.valueOf(sensorLane.getID()), "DETECTED"));
                    }
                }
            }

            if (carVisual.isSensorEventSent() && !isAtStopLine(carVisual)) {
                logicCar.leaveSensor();
                carVisual.setSensorEventSent(false);
                if (server != null) {
                    GUILane sensorLane = getLane(carVisual.getEntryBearing(), carVisual.getEntryLaneNumber());
                    if (sensorLane != null) {
                        server.sendEvent(new SimulatorEvent(TLCCommand.VEHICLE_CLEARED, String.valueOf(sensorLane.getID()), "CLEARED"));
                    }
                }
            }
            ...
            if(otherCar != null) {
                // crashes are out of scope for this project (confirmed by prof) -
                // log it but let both cars keep moving instead of freezing forever
                System.out.println("Collision detected (ignored - crashes not modeled per project scope).");
            }
    ...
    private static class CarVisual {
        ...
        private final int entryLaneNumber;
        ...
        public CarVisual(GUICar car, ImageView imageView, double speed, int laneNumber) {
            ...
            this.entryLaneNumber = laneNumber;
        ...
        public int getDestinationLane() {
            return destinationLane;
        }

        public void setTurnInfo(Bearing pendingBearing, double mergeThreshold, int destinationLane) {
